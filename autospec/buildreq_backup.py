...
class Requirements(object):
    ...
    def grab_python_requirements(self, descfile, packages, cache=False):
        ...
        for line in lines:
            ...
            if clean_python_req(line) == "mock":
                continue
            print(f"Adding additional (python) requirement: {clean_python_req(line)}")
            self.add_requires(clean_python_req(line), packages, cache=cache)
    ...
    def add_setup_py_requires(self, filename, packages, cache=False):
    ...
    def scan_for_configure(self, dirn, tname, config):
        ...
        for dirpath, _, files in os.walk(dirn):
            ...
            if "requires.txt" in files:
                self.grab_python_requirements(dirpath + "/requires.txt", config.os_packages, cache=True)
            ...
            if "requirements.txt" in files:
                self.grab_python_requirements(dirpath + "/requirements.txt", config.os_packages, cache=True)
